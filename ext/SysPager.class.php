...
class SysPager {
	...
	public $cur_page_tag = 'span';
	/** 不可用页码的标签名称，默认是 span，可以为空（如果为空，则css_unabled无效） */
	public $unabled_page_tag = 'span';
	/** 是否显示不可用的分页，默认不显示 */
	...
	/** 上一页的文字描述 */
	public $bar_prev_text = '上一页';
	/** 下一页的文字描述 */
	public $bar_next_text = '下一页';
	/** 末页的文字描述 */
	...
	public $url_html = '';
	/** 不可用的按钮（span） */
	public $css_unabled = 'pager_unabled';
	/** 当前的按钮的样式表名称(span) */
	public $css_cur = 'pager_cur';
	/** 当前选择型按钮的样式表名称（span） */
	...
	/** 总页数 */
	public $total_page = 0;
	/** 分页按钮的开始标签，不同模板可以有不同的输出，如：page_template=2 输出 <td> */
	private $first_tag = '';
	/** 分页按钮的结束标签，不同模板可以有不同的输出，如：page_template=3 输出 </td> */
	private $end_tag = '';
	 ...
	 * @return SysPager
	 */
	public function SysPager($total_num, $page_index = 1, $page_size=10)
	{
		if ($total_num > 0)
		{
	...
	private function createLinkButton($page)
	{
		...
		return $this->first_tag.'<a href="' . $url . '"'.($this->target ? ' target="' . $this->target . '"' : '').'>' . $text . '</a>'.$this->end_tag;
	}
	...
	private function createCurLinkButton()
	{
		...
		{
			return $this->first_tag.$text.$this->end_tag;
		}
		else
		{
			if ($this->page_template == 1)
			...
			{
				return $this->first_tag.'<'.$this->cur_page_tag.' class="'.$this->css_cur.'">' . $text . '</'.$this->cur_page_tag.'>'.$this->end_tag;
			}
		}
	}
