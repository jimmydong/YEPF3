			...
			try{
				...
				if($db){
					if(self::$db_log_mysql)foreach (self::$db_table as $v){
						...
						$values[] = "('".self::get_real_ip()."','".$_SERVER["SERVER_ADDR"]."','db','','',". $_SERVER["REQUEST_URI"] .",'" .addslashes($v[0]). ":" .addslashes($v[1]). "','" .addslashes($v[2]). "','" .addslashes($v[3]). "','" .addslashes(var_export($v[4], true)). "')";
					}
