 ...
 * 
 * 较原有BaseMongoRecord改进：
 * 1，可以选择默认Mongo库（注意：同时只能操作一个）
 * 2，add方法可直接插入数据（与BaseModel一致）
 * 3，数值保存在entity中（与BaseModel一致）
 * 
 * 
 * 注意：
 * 1，默认配置：WORK-ENV中[MONGODB]
 ...
 *
 */
abstract class BaseMongo implements MongoRecord
{
    public static $pool = null;				//连接池
	public static $default_conf = 'MONGODB';
    public static $connfail = null;
    public static $database = null;			//字符串型
    public static $connection = null;			//MongoClient
    public static $debug = false;				//调试开关
     ...
     * 
     * [MONGOCLOUD]
		server			= 'mongodb://docker01:27017,docker02:27017,docker03:27017'
		replicaSet		= 'rs0'
		DATABASE		= 'dbname'
		USER			= 'username'
		PASS			= 'password'
     ...
     * @param string $new
     */
    public function __construct($entity = array(), $new = true)
    {
    	if (!self::$pool[self::$default_conf]['connection']){
    		if(self::$pool[self::$default_conf]['connfail']){
    			return self::_err("MongoError:connect to {self::$default_conf} fail", \yoka\Conf::$ENV[self::$default_conf]);
    		}
    		try{
    ...
    {
    	if(! $config = \yoka\Conf::$ENV[self::$default_conf]){
    		return self::_err('Mongo配置不存在', self::$default_conf);
    	}
    	//初始化数据库连接
    	...
    	if($config['server']){
    		...
    		$connection = new \MongoClient($config['server'], array(
    	...
    	}elseif($config['USER'] && $config['PASS']){
    		//单机+用户验证
    		$connection = new \MongoClient(
    				'mongodb://'.$config['HOST'].':'.$config['PORT'],
    				array("db"=>$config['DATABASE'], "username"=>$config['USER'], "password"=>$config['PASS'])
    		);
    	}else{
    		//单机免验证
    		$connection = new \MongoClient('mongodb://'.$config['HOST'].':'.$config['PORT']);
    	}
    	if(! $connection){
    		self::$pool[self::$default_conf]['connfail'] = true;
    		return false;
    	}else{
    		self::$database = $config['DATABASE'];
    		self::$pool[self::$default_conf]['connection'] = $connection;
     ...
     * @param unknown $data
     */
    public static function _err($msg, $data){
    	\yoka\Debug::log("MongoError:connect to {self::$default_conf} fail", \yoka\Conf::$ENV[self::$default_conf]);
    	if(self::$debug){
    ...
    public function save(array $options = array(),$insert_only = false)
    {
        try{
            ...
            if ( $insert_only ){
                $ret = $collection->insert( $this->entity,  $options );
            }else{
                $ret = $collection->save(  $this->entity,  $options );
            }
            ...
            self::log_query('save',$this->entity, $options, $collection, $begin_microtime,$ret);
        }catch(\Exception $err){
            ...
            if(preg_match('/non-utf8/i',$err->getMessage())){
                ...
                foreach($this->entity as $key=>$val){
                    ...
                    else $this->entity[$key] = iconv('GBK','UTF-8//IGNORE',$val);
                }
                try{
                    $collection->save($this->entity, $options);
                }catch(\Exception $err){
     ...
     * @return object | false
     */
    static public function add($data, array $options = array())
    {
    	$className = get_called_class();
    	$obj = new $className;
    	
    	$obj->entity = $data;
    	unset($obj->entity['_id']);		//_id是保留主键字段，防止占用
    	
    	if($obj->save($options, true)){
    		return $obj;
    	}else{
    		return false;
    	}
    ...
    public function destroy($options = array())
    {
        ...
        {
            $collection = self::getCollection();
            $re = $collection->remove(array('_id' => $this->entity['_id']),$options);
        }else{
            $re = false;
        }
            ...
            if($b_data){
                foreach($b_data	as $val){
                    ...
                    if($coll_b['join_b'] == '_id'){
                        $in_array[]	=	strval($val->entity[$coll_b['join_b']]);
                        $b_data_array[strval($val->entity[$coll_b['join_b']])]	=	$val->entity;
                    }else{
                        $in_array[]	=	$val->entity[$coll_b['join_b']];
                        $b_data_array[$val->entity[$coll_b['join_b']]]	=	$val->entity;
                    }
                }
            }
            ...
            if($a_data){
                foreach($a_data as &$val){
                    foreach($coll_b['column'] as $k=>$v){
                        if($k == '_id'){
                            $val->entity[$v]	=	strval($b_data_array[$val->entity[$coll_a['join_a']]][$k]);
                        }else{
                            $val->entity[$v]	=	$b_data_array[$val->entity[$coll_a['join_a']]][$k];
                        }
                    }
                    if($coll_a['column'][0] != '*'){
                        foreach($coll_a['column'] as $k=>$v){
                            $val->entity[$v]	=	$val->entity[$k];
                        }
                    }
                }
            }
                ...
                foreach($a_data as &$val){
                    if($coll_a['column'][0] != '*'){
                        foreach($coll_a['column'] as $kc=>$vc){
                            $val->entity[$vc]	=	$val->entity[$kc];
                        }
                    }
                    if($coll_b['join_b'] == '_id'){
                        foreach($b_data as $k=>$v){
                            if(isset($val->entity[$coll_a['join_a']]) && isset($v->entity[$coll_b['join_b']]) && $val->entity[$coll_a['join_a']] == strval($v->entity[$coll_b['join_b']])){
                                foreach($coll_b['column'] as $ka=>$va){
                                    if($ka == '_id'){
                                        $val->entity[$va]	=	strval($v->entity[$ka]);
                                    }else{
                                        $val->entity[$va]	=	$v->entity[$ka];
                                    }
                                }
                                break;
                            }
                        ...
                        foreach($b_data as $k=>$v){
                            if(isset($val->entity[$coll_a['join_a']]) && isset($v->entity[$coll_b['join_b']]) && $val->entity[$coll_a['join_a']] == $v->entity[$coll_b['join_b']]){
                                foreach($coll_b['column'] as $ka=>$va){
                                    if($ka == '_id'){
                                        $val->entity[$va]	=	strval($v->entity[$ka]);
                                    }else{
                ...
                if(!is_object($val->$join_a)){
                    foreach($columns as $k=>$v){
                        if(isset($in_data[$val->$join_a])){
                            $val->entity[$v]	=	$in_data[$val->$join_a]->$k;
                        }
                    }
                }else{
                    foreach($columns as $k=>$v){
                        if(isset($in_data[strval($val->$join_a)])){
                            $val->entity[$v]	=	$in_data[strval($val->$join_a)]->$k;
                        }
                    }
                }
            }
    ...
    public function __get($property_name){
        ...
        {
            /*if (   array_key_exists($property, $this->entity) ){*/
    ...
    public function __set($property_name,$property_value){
        ...
        /*if(  static::$schema != null &&
                array_key_exists($property, static::$schema) ){ */  //changed by xwarrior@2012.5.2 for performence
        if ( isset(  static::$schema[$property] ) ){
            $this->entity[$property] = $property_value;
            return $this;
    ...
    public function __call($method, $arguments)
    {
        ...
        // Get
        //if ($prefix == "get"  && array_key_exists($property, $this->entity))
        if ($prefix == "get"  && isset( $this->entity[$property] ) )
        {
            return $this->entity[$property];
        }
        else if ($prefix == "get")
        ...
        {
            $this->entity[$property] = $arguments[0];
            return $this;
        }
        else
        {
    ...
    public function Update($overite_all = false,$upsert = false,$safe = false){
        ...
        //throw new \Exception('this is:'.$this->entity['visibility']);

        $options = array( 'upsert' => $upsert, 'multiple' => false,'safe' => $safe,'fsync' =>false, 'timeout' => static::$findTimeout  );

        $critera = array('_id' => $this->entity['_id'] );

        //added by Kit: bug report:mod on _id not allowed, _id should be removed from entity before doing update
        $_id = $this->entity['_id']; /* 保存_id属性后边再恢复  */
        unset( $this->entity['_id'] );

        //覆盖替换所有字段
        $ret = null;
        $collection = $this->getCollection();
        if ( $overite_all ){
            $ret = $collection->update($critera,  $this->entity, $options);
            $this->_id = $_id;
        }else{
            //只执行几个字段变更
            $ret = $collection->update($critera,  array( '$set' => $this->entity),$options);
            $this->_id = $_id;
        }
        //add performence log xwarrior  2012/8/8
