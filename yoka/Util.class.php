...
class Util
{
     ...
     * @author sux
     */
    static function delog_spent($tag = '')
    {
        static $begin_time = 0, $begin_memory = 0, $last_time = 0, $last_memory = 0, $i = 0;
        $time_precision = 4;
        $tag = trim((string)$tag);
        $now_time = microtime(true);
        $now_memory = round(memory_get_usage() / 1024 / 1024, 2);
        $msg = "";
        if (!$begin_time) {
            $begin_time = $last_time = $now_time;
            $begin_memory = $last_memory = $now_memory;
            $msg = "初始消耗{$tag} 时间 $now_time 内存 {$now_memory} M";
        } else {
            $spend_time = round($now_time - $last_time, $time_precision);
            $spend_memory = $now_memory - $last_memory;
            $total_time = round($now_time - $begin_time, $time_precision);
            $total_memory = $now_memory - $begin_memory;
            $msg = "当前{$tag}消耗{$spend_time}秒{$spend_memory}M 总消耗{$total_time}秒{$total_memory}M";
            $last_time = $now_time;
            $last_memory = $now_memory;
        }
        $i++;
        \yoka\Debug::log("spent{$i}", $msg);
    }
}