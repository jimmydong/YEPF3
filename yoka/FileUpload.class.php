...
class FileUpload{
	...
	public static function init($file_path_upload='', $url_path_upload=''){
		...
		}elseif(defined('FILE_PATH_UPLOAD') && file_exists(FILE_PATH_UPLOAD)){ //历史版本兼容
			self::$file_path_upload = FILE_PATH_UPLOAD;
	...
	public static function uploadFile($name, $full = false){
		...
		if(! self::$file_ext_allowed[pathinfo($real_path, PATHINFO_EXTENSION)]) return \yoka\YsError::error('上传失败：文件类型错误');
